...
We selected 2 PC datasets with copy number segmentation data from cBioPortal:

- http://www.cbioportal.org/study/summary?id=prad_su2c_2019 (Metastatic tumors)
...
```{r}
library(sigminer)
load("../output/Sig.CNV.su2c.RData")
...
```{r, fig.width=12, fig.height=7}
show_sig_profile(Sig.CNV.su2c, style = "cosmic", method = "W", normalize = "feature")
```
...
```{r}
show_sig_exposure(Sig.CNV.su2c, rm_space = TRUE, style = "cosmic")
```
...
sim_su2c = get_sig_similarity(Sig.CNV.su2c, Sig.CNV.seqz.W, normalize = "feature", pattern_to_rm = "BoChr")
sim_mskcc = get_sig_similarity(Sig.CNV.mskcc, Sig.CNV.seqz.W, normalize = "feature",  pattern_to_rm = "BoChr")

rownames(sim_su2c$similarity) = paste(rownames(sim_su2c$similarity), "(SU2C)")
rownames(sim_mskcc$similarity) = paste(rownames(sim_mskcc$similarity), "(MSKCC)")
```


```{r}
pheatmap::pheatmap(sim_su2c$similarity, cluster_cols = F, cluster_rows = F, display_numbers = TRUE)
```


```{r}
pheatmap::pheatmap(sim_mskcc$similarity, cluster_cols = F, cluster_rows = F, display_numbers = TRUE)
```
...
expo_su2c = get_sig_exposure(Sig.CNV.su2c)
```
...
su2c_patient = readr::read_tsv("../data/prad_su2c/data_clinical_patient.txt", comment = "#")
su2c_sample  = readr::read_tsv("../data/prad_su2c/data_clinical_sample.txt", comment = "#")
```
...
dat_su2c2 = dat_su2c %>% 
  dplyr::select(sample:PATIENT_ID, OS_STATUS, OS_MONTHS) %>% 
  dplyr::mutate(
    OS_STATUS = ifelse(OS_STATUS == "LIVING", 0, 1)
  ) %>% 
  na.omit() %>% 
...
> Of note, here we does not confirm that `Sig1` is `CN-Sig1`,
> `Sig2` is `CN-Sig2`, ..., and `Sig5` is `CN-Sig5` shown in major result part. 

```{r}
show_forest(dat_su2c2, covariates = cols_to_sig,
            time = "OS_MONTHS", status = "OS_STATUS", merge_models = TRUE)
```

All samples in su2c dataset are metastatic, the result seems to show that exposure is not relavant to prognosis of metastatic samples. Here we check it with mskcc dataset by splitting it into group 'primary' and 'metastatic'.
