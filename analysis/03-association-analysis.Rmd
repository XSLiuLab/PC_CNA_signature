...
```{r}
summary(tidy_data.seqz.gene$measure)
```


```{r, fig.height=8, fig.width=9}
## 用梯度颜色解决 CNV 和 SNV Exposure 差异大的
## 问题，但颜色条白色没有对应到0
## 很奇怪，搞了很久没弄好，请教下别人
...
sc <- scale_colour_gradientn(
  ...
  values = scales::rescale(c(
    tidy_data.seqz.gene$measure %>% min(na.rm = TRUE),
    0,
    20,
    tidy_data.seqz.gene$measure %>% max(na.rm = TRUE))
  ))
...
```{r}
summary(tidy_data.seqz.pathways$measure)
```
...
sc <- scale_colour_gradientn(
  colors = c("blue", "white", "yellow", "red"),
  values = scales::rescale(c(
    tidy_data.seqz.pathways$measure %>% min(na.rm = TRUE),
    0, 
    40,
    tidy_data.seqz.pathways$measure %>% max(na.rm = TRUE))
  ))
