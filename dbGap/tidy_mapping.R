...
cols = c("AvgSpotLen", "InsertSize", "Platform", "Instrument", "LibraryLayout", "Run", "SRA_Sample",
...
mapping_df = paired_ids %>% 
    rowwise() %>% 
    do(df = filter(runInfo, gap_accession==.$gap_accession, 
...
tmp = tmp %>% 
    ...
    mutate(SUBJID = ifelse(startsWith(SUBJID, "STID"), 
                           SUBJID,
                           gsub("[^0-9-]", "", SUBJID))) # Remove any non-numeric characters
